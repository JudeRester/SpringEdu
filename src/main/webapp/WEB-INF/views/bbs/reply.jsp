...
// 대댓글작성 
   ...
   //요청페이지 댓글목록 가져오기
   function replyList(recurrPage){
      
      var str = "";
      
      $.ajax({
         type:"GET",
         url:"/rbbs/map/"+bnum+"/"+recurrPage,
         dataType: "JSON",
         success: function(data){
            $.each(data.item, function(idx,rec){
               ...
               if(rec.rid == login_id){
                  ...
                  str   +=    "<button id='b0' class='btn btn-outline-primary btn-sm'>댓글</button>"                      
                       +    "<button id='b3' class='btn btn-outline-danger btn-sm'>좋아요"+"<span class='badge badge-pill badge-primary'>" + rec.rgood + "</span></button>"
                      +    "<button id='b4' class='btn btn-outline-warning btn-sm'>나빠요"
                      +      "<span class='badge badge-pill badge-primary'>" + rec.rbad + "</button>" 
                      +    "</li></span>";      
               }
   ...
   function showPageList(pc){
      ...
      if(pc.prev){
         ...
         // 이전
         str += "<li class='page-item'><a href='" + (pc.startpage-1) + "'>" + "◁</a></li>";   
      }
      
      for(var i=pc.startpage, end=pc.endpage; i<=end; i++){
         // 
      ...
      if(pc.next){
         // 다음
         str += "<li class='page-item'><a href='" + (pc.endpage+1) + "'>" + "▷</a></li>";
         // 마지막
         str += "<li class='page-item'><a href='" + (pc.lastpage) + "'>" + "▶</a></li>";   
      }   
   ...
   <table class="table table-sm">
      <tr class="table-primary">
         <td class="align-middle" style='width: 120px;'>
            ${user.name }
            <input type="hidden" id="writer" value="${user.name }">   
            <input type="hidden" name="rid" value="${user.username }">
         </td>
         <td><textarea name="" id="replyContent" cols="120" rows="3"></textarea></td>
         <td class="align-middle" style='width: 120px;'><button id="replyBtn" class="btn btn-info btn-sm">댓글작성</button></td>
    </tr>
